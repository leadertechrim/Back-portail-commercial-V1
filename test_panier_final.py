...
def get_panier_items():
    """Récupérer tous les éléments du panier"""
    try:
        # Récupérer les éléments du panier
        panier_items = list(panier_col.find({}).sort("created_at", -1))
        
        # Convertir les ObjectId en string
        for item in panier_items:
            item["_id"] = str(item["_id"])
        
        return jsonify(panier_items), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def add_to_panier():
    ...
    try:
        ...
        return jsonify({"message": "Élément ajouté au panier avec succès", "item": panier_item}), 201
...
@app.route("/api/panier/test", methods=["GET"])
def test_panier_connection():
    """Route de test spécifique pour le panier"""
    try:
        # Test de connexion MongoDB
        client.admin.command('ping')
        
        # Compter les documents dans le panier
        count = panier_col.count_documents({})
        
        return jsonify({
            "status": "Panier accessible",
            "message": "Connexion panier réussie",
            "total_items": count,
            "timestamp": datetime.utcnow().isoformat()
        })
    except Exception as e:
        return jsonify({
            "status": "Erreur panier",
